    ...
    *result_count = 0;
    ...
    for (int i = 0; i < queries_rows; i++) {
        ...
        if (type == 1) {
            ...
            sizes[seqIndex]++;
            seq[seqIndex] = (int*)realloc(seq[seqIndex], sizes[seqIndex] * sizeof(int));
            seq[seqIndex][sizes[seqIndex] - 1] = y;

        } else if (type == 2) {
            
            int idx = y % sizes[seqIndex];
            lastAnswer = seq[seqIndex][idx];

           
            (*result_count)++;
            results = (int*)realloc(results, (*result_count) * sizeof(int));
